# 实施计划

- [x] 1. 搭建项目结构和开发环境





  - 为前端（React）和后端（Flask）组件创建目录结构
  - 为前端初始化package.json，包含React、TypeScript和所需依赖
  - 为后端设置Python虚拟环境和requirements.txt
  - 配置开发工具（ESLint、Prettier、Black、flake8）
  - _需求: 所有需求的基础_

- [x] 2. 实现后端核心基础设施





  - [x] 2.1 创建具有基本配置的Flask应用程序


    - 设置支持CORS的Flask应用
    - 配置文件上传设置和大小限制
    - 使用蓝图创建基本项目结构
    - _需求: 1.5, 5.4_

  - [x] 2.2 实现数据模型和存储


    - 创建MenuData、Meal和MenuItem类
    - 实现用于内存数据管理的MenuStorage类
    - 添加数据验证和序列化方法
    - _需求: 1.3, 2.3_

  - [ ]* 2.3 为数据存储往返编写属性测试
    - **属性 3: 数据存储往返**
    - **验证: 需求 1.3**

- [-] 3. 实现Excel文件处理


  - [x] 3.1 创建Excel解析器模块



    - 使用pandas和openpyxl实现ExcelParser类
    - 添加文件格式验证和内容提取
    - 处理各种Excel结构和日期格式
    - _需求: 1.1, 1.2, 1.4_

  - [ ]* 3.2 为文件格式验证编写属性测试
    - **属性 1: 文件格式验证一致性**
    - **验证: 需求 1.1, 1.4, 5.1**

  - [ ]* 3.3 为Excel解析完整性编写属性测试
    - **属性 2: Excel解析完整性**
    - **验证: 需求 1.2**

  - [ ]* 3.4 为Excel解析健壮性编写属性测试
    - **属性 11: Excel解析健壮性**
    - **验证: 需求 5.2, 5.5**

- [x] 4. 创建后端API端点




  - [x] 4.1 实现文件上传端点


    - 创建带有文件验证的POST /api/upload端点
    - 添加文件大小检查和安全验证
    - 与Excel解析器和数据存储集成
    - _需求: 1.1, 1.2, 1.3, 1.4, 1.5_

  - [x] 4.2 实现菜单检索端点


    - 创建带有日期参数的GET /api/menu端点
    - 创建用于可用日期的GET /api/dates端点
    - 为缺失菜单数据添加回退逻辑
    - _需求: 2.1, 2.2, 2.4, 3.2_

  - [ ]* 4.3 为文件大小限制执行编写属性测试
    - **属性 4: 文件大小限制执行**
    - **验证: 需求 1.5, 5.4**

  - [ ]* 4.4 为回退菜单选择编写属性测试
    - **属性 5: 回退菜单选择**
    - **验证: 需求 2.2**

- [x] 5. 检查点 - 确保后端测试通过




  - 确保所有测试通过，如有问题请询问用户。

- [-] 6. 设置前端React应用程序


  - [x] 6.1 创建React TypeScript项目结构


    - 使用TypeScript和所需依赖初始化React应用
    - 设置组件目录结构和路由
    - 配置构建工具和开发服务器
    - _需求: 4.1, 4.2, 4.4_

  - [x] 6.2 实现核心React组件




    - 创建具有状态管理的App组件
    - 实现基本组件结构和属性接口
    - 为状态和API通信设置React hooks
    - _需求: 2.1, 3.1, 3.5_


- [x] 7. 实现文件上传功能




  - [x] 7.1 创建MenuUpload组件

    - 构建支持拖放的文件选择界面
    - 添加上传进度指示和错误处理
    - 与后端上传API集成
    - _需求: 1.1, 1.4, 1.5_


  - [x] 7.2 添加上传验证和反馈






    - 实现客户端文件格式验证
    - 为上传成功和错误添加用户反馈
    - 处理上传进度和加载状态
    - _需求: 1.4, 5.1_

- [x] 8. 实现菜单显示功能







  - [x] 8.1 创建MenuDisplay组件



    - 构建具有响应式设计的菜单卡片布局
    - 实现餐次分组和时间组织
    - 为无菜单数据添加空状态处理
    - _需求: 2.3, 2.4, 2.5, 4.1, 4.2_

  - [ ]* 8.2 为菜单显示完整性编写属性测试
    - **属性 6: 菜单显示完整性**
    - **验证: 需求 2.3**

  - [ ]* 8.3 为空日期处理编写属性测试
    - **属性 7: 空日期处理**
    - **验证: 需求 2.4**

  - [ ]* 8.4 为餐次组织一致性编写属性测试
    - **属性 8: 餐次组织一致性**
    - **验证: 需求 2.5**

- [x] 9. 实现日期导航功能









  - [x] 9.1 创建DateSelector组件



    - 构建带有导航控件的日期选择器界面
    - 实现日期选择和状态管理
    - 添加可用日期高亮和验证
    - _需求: 3.1, 3.2, 3.3, 3.5_

  - [ ]* 9.2 为日期选择同步编写属性测试
    - **属性 9: 日期选择同步**
    - **验证: 需求 3.2**

  - [ ]* 9.3 为日期状态持久性编写属性测试
    - **属性 10: 日期状态持久性**
    - **验证: 需求 3.5**

- [x] 10. 实现响应式设计和样式





  - [x] 10.1 创建与Bootstrap集成的响应式CSS

    - 实现移动优先的响应式设计
    - 为菜单卡片和组件添加一致的样式
    - 创建现代配色方案和排版
    - _需求: 4.1, 4.2, 4.4, 4.5, 6.1, 6.2_

  - [ ]* 10.2 为UI样式一致性编写属性测试
    - **属性 12: UI样式一致性**
    - **验证: 需求 6.2**

- [x] 11. 集成前端和后端






  - [x] 11.1 将React组件连接到Flask API


    - 使用Axios实现API服务层
    - 添加错误处理和重试逻辑
    - 将所有组件连接到后端端点
    - _需求: 2.1, 2.2, 3.2, 3.4_

  - [x] 11.2 添加全面的错误处理


    - 实现前端错误边界和通知
    - 添加加载状态和用户反馈
    - 处理网络连接问题
    - _需求: 1.4, 5.2, 6.3_

- [x] 12. 检查点 - 确保所有测试通过






  - 确保所有测试通过，如有问题请询问用户。

- [ ]* 13. 添加生产优化
  - [ ]* 13.1 优化文件上传和处理
    - 添加文件压缩和分块上传支持
    - 为频繁访问的菜单数据实现缓存
    - 优化Excel解析性能
    - _需求: 5.3, 3.4_

  - [ ]* 13.2 添加安全增强
    - 实现额外的文件内容验证
    - 为API端点添加速率限制
    - 增强错误日志记录和监控
    - _需求: 5.1, 5.2_

- [ ]* 14. 最终测试和验证
  - [ ]* 14.1 为完整工作流编写集成测试
    - 测试文件上传到菜单显示的工作流
    - 测试日期导航和菜单检索
    - 测试错误场景和恢复

  - [ ]* 14.2 使用Cypress编写端到端测试
    - 在浏览器环境中测试完整的用户工作流
    - 验证不同屏幕尺寸上的响应式设计
    - 测试可访问性和可用性要求

- [x] 15. 最终检查点 - 确保所有测试通过

  - 确保所有测试通过，如有问题请询问用户。

- [x] 16. 增强功能实现 - WPS表格支持和时区处理

  - [x] 16.1 添加WPS表格文件(.et)格式支持
    - 实现多种解析策略：pandas直接读取、xlrd引擎、格式转换、CSV方式
    - 添加xlrd==2.0.1依赖支持WPS表格文件
    - 更新文件扫描器支持*.et文件模式
    - _需求: 7.1, 7.2, 7.3, 7.4, 7.5_

  - [x] 16.2 实现Docker环境变量时区支持
    - 添加TZ=Asia/Shanghai环境变量到Docker配置
    - 创建统一的时区处理工具模块(backend/app/utils/timezone.py)
    - 修改所有使用datetime.now()的代码使用时区工具
    - _需求: 10.1, 10.2, 10.3, 10.4, 10.5_

  - [x] 16.3 将项目文档中文化
    - 翻译所有README、部署指南、贡献指南等文档为中文
    - 更新代码注释和错误信息为中文
    - 确保用户界面文本使用中文

- [x] 17. 修复Excel解析问题

  - [x] 17.1 修复基于星期格式菜单的解析问题
    - 修复星期列检测逻辑，正确识别所有星期列
    - 实现智能餐次推断逻辑（基于"类别"行数量）
    - 修复29、30、31号菜单数据丢失问题
    - _需求: 9.1, 9.2, 9.3, 9.4, 9.5_

  - [x] 17.2 修复菜品分类逻辑问题
    - 实现分类延续机制，正确处理pandas的NaN值
    - 修复跨多行分类的处理（如荤类包含多个菜品）
    - 改进分类识别算法，区分"免费例汤"和"炖罐"等相似分类
    - _需求: 8.5（新增验收标准）_

- [x] 18. 修复前端显示问题

  - [x] 18.1 修复月历日期标记错位问题
    - 修复formatDateString函数使用本地时区格式化
    - 解决toISOString()导致的UTC时区偏移问题
    - 确保菜单数据标记在正确的日期上显示
    - _需求: 10.3（时区相关）_

  - [x] 18.2 实现菜品和分类的原始顺序排序
    - 在MenuItem模型中添加order和category_order字段
    - 在Excel解析过程中记录分类和菜品的出现顺序
    - 在数据序列化时按照原始顺序排序
    - _需求: 8.1, 8.2, 8.3, 8.4_

- [x] 19. 创建项目文档和更新日志

  - [x] 19.1 创建完整的更新日志
    - 记录所有重要的功能更新、问题修复和改进优化
    - 包括版本说明、更新类型定义、贡献指南
    - 使用中文编写所有文档内容
    - _文档完善_

- [x] 20. 修复GitHub构建测试问题

  - [x] 20.1 修复MenuItem序列化测试
    - 更新测试用例以包含新添加的order和category_order字段
    - 确保测试期望值与实际序列化结果一致
    - 验证所有60个测试通过
    - _测试修复_