...
const MenuDisplay: React.FC<MenuDisplayProps> = memo(
  ({ menuData, selectedDate, loading = false }) => {
    // 格式化餐次时间显示
    const formatMealTime = useCallback((time: string): string => {
      return time;
    }, []);

    // 获取菜品图标
    const getFoodIcon = useCallback(
      (foodName: string, category: string): string => {
        // 根据菜品名称和分类返回合适的图标
        ...
        // 根据菜品名称匹配图标
        if (name.includes('鸡') || name.includes('鸭') || name.includes('鹅'))
          return '🐔';
        if (name.includes('猪') || name.includes('肉') || name.includes('排骨'))
          return '🥩';
        ...
        if (name.includes('面') || name.includes('粉') || name.includes('饺子'))
          return '🍜';
    ...
    const getMealTypeName = useCallback((type: string): string => {
      const mealNames = {
        breakfast: '早餐',
        lunch: '午餐',
        dinner: '晚餐',
    ...
    const groupItemsByCategory = useCallback(
      (items: MenuItem[]): { [category: string]: MenuItem[] } => {
        const grouped: { [category: string]: MenuItem[] } = {};

        items.forEach(item => {
          const category =
            item.category && item.category !== '<NA>' ? item.category : '其他';
          if (!grouped[category]) {
            grouped[category] = [];
          }
    ...
    // 检查是否有档口特色菜品
    const hasSpecialtyDishes = useCallback(
      (menuData: MenuData | null): boolean => {
    ...
    // 获取所有档口特色菜品
    const getSpecialtyDishes = useCallback(
      (menuData: MenuData | null): MenuItem[] => {
    ...
    const renderCategorySection = useCallback(
      (category: string, items: MenuItem[]) => {
        ...
        return (
          <div
            key={category}
            className={`food-category ${isSpecialty ? 'specialty-category' : ''}`}
          >
            <div
              className={`category-title ${isSpecialty ? 'specialty-title' : ''}`}
            >
        ...
        return (
          <div key={index} className="meal-card">
            ...
            <div className="meal-stats">
              <span>
                <i className="bi bi-list-ul me-1"></i>共 {meal.items.length}{' '}
                道菜品
              </span>
              <span>
    ...
    const sortedMeals = useMemo(() => {
      ...
      return [...menuData.meals].sort((a, b) => {
    ...
    const hasSpecialty = useMemo(
      () => hasSpecialtyDishes(menuData),
      [menuData, hasSpecialtyDishes]
    );

    const specialtyDishes = useMemo(
      () => getSpecialtyDishes(menuData),
      [menuData, getSpecialtyDishes]
    );

    // 渲染档口特色横幅
    const renderSpecialtyBanner = useCallback(() => {
    ...
    return (
      <div
        ...
        role="region"
        aria-label={`菜单详情: ${formatDate(selectedDate)}`}
      >
        {/* 档口特色横幅 */}
